...
public class a2Part1 {
    ...
    public static void main(String[] _ignored) {
        ...
        double [][][] deltaWeights = nn.backward_propagate_error(instances[0], outputs[0], outputs[1], integer_encoded[0]);
        nn.update_weights(initial_output_layer_weights, initial_hidden_layer_weights);
        ...
        int[] test_integer_encoded = label_encoder.intEncode(labels_test);
        int[] prediction = nn.predict(instances_test);
        int correctClasses = 0;
        for (int i = 0; i < instances_test.length; i++) {
       
      
            if (prediction[i] == test_integer_encoded[i]) {
            	correctClasses++;
            }
        
        }
        double acc = (double)correctClasses/instances_test.length;
